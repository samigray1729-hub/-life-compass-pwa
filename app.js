...
async function addProgress(event) {
  ...
  await LifeDB.put("progress", { id: uid(), gaugeId: form.elements.gaugeId.value, amount, memo: form.elements.memo.value.trim(), date: selectedDate, createdAt: new Date().toISOString() });
  form.closest("dialog").close();
  await renderGauges();
